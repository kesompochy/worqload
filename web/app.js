...
// The gh actions deliberately use an inline panel rather than a modal: the run
// log stays on screen alongside the rest of the session, and (because the
// server records each run as an action_invoked event) it survives a reload.
// The confirmation step is the explicit "Confirm & Run" button next to the
// highlighted confirmMessage — the panel is the gate, not a separate dialog.
function renderActionPanelHtml() {
