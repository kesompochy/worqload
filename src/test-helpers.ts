...
export function fakeWorktreeOps(): WorktreeOps {
  return {
    ...
    // Reading, writing, creating, deleting, and renaming a single file are
    // pure fs (path-escape checks, binary sniff, size limit) with no git, so
    // the production implementation is the fake too.
    readWorktreeFile: realWorktreeOps.readWorktreeFile,
    writeWorktreeFile: realWorktreeOps.writeWorktreeFile,
    createWorktreeFile: realWorktreeOps.createWorktreeFile,
    deleteWorktreeFile: realWorktreeOps.deleteWorktreeFile,
    renameWorktreeFile: realWorktreeOps.renameWorktreeFile,
