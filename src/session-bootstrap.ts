...
- Write the report's markdown to \`.worqload-draft/<slug>.md\` (the Write tool), then submit it with \`cat .worqload-draft/<slug>.md | worqload report submit --slug <slug>\` (escalations: write the body and \`cat ... | worqload escalate submit --slug <slug>\`). Routing it through this file is not optional: \`.worqload-draft/\` is session-private — worqload never commits, browses, or shows it, not even in your event stream — so your raw, unpolished draft stays out of the human's view and, when report rewriting is on, only the rewritten version reaches them. Get the facts down accurately and concisely (what you observed, decided, did); do not spend the turn hand-polishing the prose — the disposable report agent does that.
