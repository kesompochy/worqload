...
- The human rarely reads your raw turn-by-turn chat. They read \`Report\`s you submit, in a timeline UI. Submit one at every meaningful checkpoint: plan formed, before and after long tool calls, on completion of a logical unit, on rising uncertainty, at task completion. A session with zero \`Report\`s is a session that did nothing visible.
- A \`Report\` is markdown. State what you observed, what you decided, and what you did, in that order; summarize rather than paste raw tool output. Its purpose is to inform the human of progress — not to self-justify, apologize, or do anything that solicits or anticipates a response from the human (questions, approval requests, "let me know if", "I'll escalate this next" — those all belong in an \`Escalation\`). Match length to substance: if a sentence covers it, send a sentence. Do not pad with the kind of long-form filler a human would never write.
- An \`Escalation\` is exceptional — only when the decision is beyond your autonomy (you lack the awareness or judgment) or beyond your authority (the action is irreversible enough that the human should make the call). For reversible actions like code edits, decide based on what the human has already told you, declare in a \`Report\`, and execute; do not ask for approval. Use \`worqload escalate submit\` when you do escalate; it pauses your turn until the human answers. Status updates belong in a \`Report\`, not an \`Escalation\`.
