...
// Image extensions the file viewer renders inline. Detection is by extension
// rather than content sniffing because the extension is also what the browser
// needs as the response content-type. SVG is intentionally absent: it carries
// no NUL bytes, so it stays `text` and remains viewable and editable as source.
...
export async function readWorktreeFile(
  ...
  const imageMediaType = IMAGE_MEDIA_TYPE_BY_EXTENSION[extname(relPath).toLowerCase()];
  if (imageMediaType) return { kind: "image", bytes, mediaType: imageMediaType };
...
export type WorktreeFileWriteResult =
  | { kind: "ok" }
  | { kind: "not-found" }
  | { kind: "not-a-file" }
  | { kind: "denied" };

// Overwrites a single existing file inside the worktree with `content`. relPath
// is worktree-relative and runs the same lexical + symlink escape checks as
// `readWorktreeFile`, so a crafted ?path= can't write outside the worktree.
// Only existing regular files are writable: a missing path is `not-found` (the
// editor opens a file before saving it, so the target always exists) and a
// directory is `not-a-file`. Refusing to create new paths also keeps the
// symlink check meaningful — it can only run against a path that resolves.
export async function writeWorktreeFile(
  worktreePath: string,
  relPath: string,
  content: string,
): Promise<WorktreeFileWriteResult> {
  const root = resolve(worktreePath);
  const abs = resolve(root, relPath);
  if (abs !== root && !abs.startsWith(root + sep)) return { kind: "denied" };
  ...
  if (!statSync(real).isFile()) return { kind: "not-a-file" };

  await Bun.write(real, content);
  return { kind: "ok" };
}

export type WorktreeFileCreateResult =
  | { kind: "ok" }
  | { kind: "exists" }
  | { kind: "denied" };

// Creates a new file at a worktree-relative path, making parent directories as
// needed. `isSafeRepoRelativePath` rejects `..`/absolute paths up front, and
// the nearest existing ancestor directory is realpath-checked so a symlinked
// path segment can't redirect the write outside the worktree. An already-taken
// path is `exists`: overwriting is the editor's job (`writeWorktreeFile`), so
// this only ever brings a brand-new file into being.
export async function createWorktreeFile(
  worktreePath: string,
  relPath: string,
  content: string,
): Promise<WorktreeFileCreateResult> {
  if (!isSafeRepoRelativePath(relPath)) return { kind: "denied" };
  const root = resolve(worktreePath);
  const abs = resolve(root, relPath);
  if (!abs.startsWith(root + sep)) return { kind: "denied" };
  if (existsSync(abs)) return { kind: "exists" };

  let ancestor = dirname(abs);
  while (!existsSync(ancestor)) ancestor = dirname(ancestor);
  const realAncestor = realpathSync(ancestor);
  const realRoot = realpathSync(root);
  if (realAncestor !== realRoot && !realAncestor.startsWith(realRoot + sep)) return { kind: "denied" };
...
export type WorktreeFileDeleteResult =
  | { kind: "ok" }
  | { kind: "not-found" }
  | { kind: "not-a-file" }
  | { kind: "denied" };

// Deletes a single file from inside the worktree, with the same lexical +
// symlink escape checks as `readWorktreeFile`. Only regular files are
// removable: a directory is `not-a-file` (removing a whole tree is beyond
// "simple file editing") and a missing path is `not-found`.
export async function deleteWorktreeFile(
  worktreePath: string,
  relPath: string,
): Promise<WorktreeFileDeleteResult> {
  const root = resolve(worktreePath);
  const abs = resolve(root, relPath);
  if (abs !== root && !abs.startsWith(root + sep)) return { kind: "denied" };
...
export type WorktreeFileRenameResult =
  | { kind: "ok" }
  | { kind: "not-found" }
  | { kind: "not-a-file" }
  | { kind: "exists" }
...
// Renames a worktree file: the source is checked like `readWorktreeFile`
// (must be an existing regular file inside the worktree), the destination like
// `createWorktreeFile` (a brand-new path, parent directories created as
// needed). A destination that already exists is `exists` — rename never
// clobbers another file.
export async function renameWorktreeFile(
  worktreePath: string,
  fromRelPath: string,
  toRelPath: string,
): Promise<WorktreeFileRenameResult> {
  const root = resolve(worktreePath);

  const fromAbs = resolve(root, fromRelPath);
  if (fromAbs !== root && !fromAbs.startsWith(root + sep)) return { kind: "denied" };
  let realFrom: string;
  let realRoot: string;
  try {
    realFrom = realpathSync(fromAbs);
    realRoot = realpathSync(root);
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === "ENOENT") return { kind: "not-found" };
    throw err;
  }
  if (realFrom !== realRoot && !realFrom.startsWith(realRoot + sep)) return { kind: "denied" };
  if (!statSync(realFrom).isFile()) return { kind: "not-a-file" };

  if (!isSafeRepoRelativePath(toRelPath)) return { kind: "denied" };
  const toAbs = resolve(root, toRelPath);
  if (!toAbs.startsWith(root + sep)) return { kind: "denied" };
  if (existsSync(toAbs)) return { kind: "exists" };
  let ancestor = dirname(toAbs);
  while (!existsSync(ancestor)) ancestor = dirname(ancestor);
  const realAncestor = realpathSync(ancestor);
  if (realAncestor !== realRoot && !realAncestor.startsWith(realRoot + sep)) return { kind: "denied" };
...
export interface WorktreeOps {
  ...
  writeWorktreeFile(worktreePath: string, relPath: string, content: string): Promise<WorktreeFileWriteResult>;
  createWorktreeFile(worktreePath: string, relPath: string, content: string): Promise<WorktreeFileCreateResult>;
  deleteWorktreeFile(worktreePath: string, relPath: string): Promise<WorktreeFileDeleteResult>;
  renameWorktreeFile(worktreePath: string, fromRelPath: string, toRelPath: string): Promise<WorktreeFileRenameResult>;
